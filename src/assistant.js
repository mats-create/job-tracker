...
var CURATED_PROMPTS=[
  {category:"CV & profil",title:"Hitta luckor mot en roll",prompt:"Jag funderar på rollen [måltitel] som jag inte sökt än. Baserat på en typisk sådan tjänst — vilka kompetenser eller erfarenheter saknar jag troligen, och hur skulle jag kunna formulera om befintlig erfarenhet för att bättre matcha?"},
  {category:"CV & profil",title:"Stärk en svag mening",prompt:"Titta på [sektion] i mitt CV. Finns det passiva eller vaga formuleringar som kan bli starkare, utan att du hittar på siffror eller resultat som inte finns?"},
  {category:"Jobbmatchning",title:"Hur fungerar matchningsscoret?",prompt:"Förklara hur matchningsscoret räknas fram rent generellt. Vilka faktorer väger tyngst, och vad kan jag göra för att höja mitt genomsnittliga score över tid?"},
  {category:"Jobbmatchning",title:"Hjälp mig prioritera",prompt:"Av mina nuvarande New-jobb, vilka tre borde jag prioritera att granska först och varför?"},
  {category:"Personligt brev",title:"Anpassa tonen",prompt:"Det här brevet känns för formellt/informellt för rollen. Kan du justera tonen utan att ändra innehållet?"},
  {category:"Intervjuförberedelse",title:"Vanliga frågor för rollen",prompt:"Vilka intervjufrågor är typiska för [jobbtitel], och hur skulle jag kunna svara baserat på mitt CV?"},
  {category:"Intervjuförberedelse",title:"Träna på STAR-svar",prompt:"Hjälp mig formulera ett STAR-svar (Situation, Task, Action, Result) för min erfarenhet av [ämne från CV], till en fråga om [kompetens]."},
  {category:"Intervjuförberedelse",title:"Analysera feedback",prompt:"Här är feedback jag fick efter en intervju: [klistra in]. Vilka mönster ser du, och vad bör jag öva på inför nästa?"},
  {category:"Personlighetstester",title:"Förklara ett testformat",prompt:"Jag ska göra ett [Big Five/Hogan/DISC/etc]-test för en process. Vad mäter det, och finns det något jag bör tänka på utifrån min profil?"},
  {category:"Karriärstrategi",title:"Gap-analys vid karriärbyte",prompt:"Jag siktar på en övergång från [nuvarande roll] till [målroll]. Vilka delar av min erfarenhet överförs direkt, och vilka behöver jag bygga vidare på?"},
];
  ...
  return <div style={{position:"fixed",inset:0,zIndex:650,background:"rgba(0,0,0,0.45)",display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={onClose}>
    <div style={{background:C.surface,borderRadius:18,padding:22,width:"min(420px,96vw)",boxShadow:"0 8px 36px rgba(0,0,0,0.25)"}} onClick={function(e){e.stopPropagation();}}>
      ...
      <div style={{display:"flex",gap:10}}>
        <Btn variant="primary" onClick={function(){if(title.trim())onSave({title:title.trim(),category:category,prompt:initialText});}} style={{flex:1,fontSize:14,minHeight:44}} disabled={!title.trim()}>Save</Btn>
