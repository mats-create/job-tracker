...
function AccessDeniedScreen({email,onSignOut}){
  ...
  return <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:C.bg,padding:24}}>
    <div style={{width:"100%",maxWidth:420,textAlign:"center"}}>
      ...
      <p style={{fontSize:14,color:C.textHint,lineHeight:1.6,marginBottom:24}}>
        If you received an invitation, make sure you're signing in with the correct Google account.
      </p>
      <a href={mailHref} style={{display:"block",width:"100%",padding:"13px",borderRadius:12,background:C.primary,color:"#fff",fontWeight:700,fontSize:15,textDecoration:"none",marginBottom:12,boxSizing:"border-box"}}>
        ✉ Request access
      </a>
      <button onClick={onSignOut} style={{fontSize:14,fontWeight:600,padding:"12px 28px",borderRadius:12,border:"1.5px solid "+C.border,background:C.surface,color:C.textSecondary,cursor:"pointer",fontFamily:"inherit",width:"100%"}}>
...
function App(){
  ...
  // Rev: 2026-06-17 — One-time cleanup: firstSeen/lastSeen used to live on
  //   users/{uid} (see writeUserMeta history). That broke admin reporting when
  //   one email mapped to >1 uid, so login tracking moved to loginMeta/{email}
  //   instead. This function deletes the now-unused fields from the CURRENT
  //   user's own users/{uid} doc on login, so old data doesn't linger and
  //   confuse anyone reading Firestore directly. It writes nothing if the
  //   fields are already gone, so it becomes a harmless no-op on its own.
  //
  //   REMOVE THIS FUNCTION AND BOTH CALL SITES ABOVE once every known user
  //   (check admin.html — currently 4 allowlist entries) has logged in at
  //   least once after this date. At that point every users/{uid} doc will
  //   already be clean and this code has nothing left to do.
  function cleanupLegacyUserMeta(sdk, user){
    var metaRef=sdk.doc(sdk.db,"users",user.uid);
    sdk.getDoc(metaRef).then(function(snap){
      if(!snap.exists()) return;
      var data=snap.data();
      if(!("firstSeen" in data) && !("lastSeen" in data)) return; // already clean — no-op
      var updates={};
      if("firstSeen" in data) updates.firstSeen=sdk.deleteField();
