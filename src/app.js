...
function AppShell({user,onSignOut}){
  ...
  return <div style={{display:"flex",minHeight:"100vh",background:C.background}}>
    ...
    <div style={{flex:1,minWidth:0,display:"flex",flexDirection:"column",paddingBottom:isMobile?"80px":0}}>
...
function App(){
  ...
  useEffect(function(){
    var auth=window.__jt_auth;
    if(!auth){ setUser(null); setAuthChecked(true); return; }
    var unsub=auth.onAuthStateChanged(function(u){ setUser(u||null); setAuthChecked(true); });
    return function(){ if(unsub) unsub(); };
  },[]);

  async function signIn(){
    var auth=window.__jt_auth;
    var provider=new window.__jt_GoogleAuthProvider();
