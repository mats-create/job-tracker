        ...
        {groupedTabs.map(function(gp,i){
          return <div key={gp.group.id} style={{marginBottom:4}}>
            ...
            {gp.tabs.map(function(t){
              ...
              var isGabbi=t.id==="assistant";
              // Gabbi: same shape as other items but with a soft warm green tint
              // resting: slightly greener than active-tab bg; active: standard active style
              var gabbiStyle=isGabbi&&!on?{
                background:"#D4EAE2",
                color:"#2E6B54",
                fontWeight:600,
              }:isGabbi&&on?{
              ...
              return <button key={t.id} className={"jt-sb-item"+(on?" active":"")} onClick={function(){pickTab(t.id);}} title={collapsed?t.label:""} style={Object.assign({},{display:"flex",alignItems:"center",gap:12,width:"calc(100% - 12px)",margin:"2px 6px",padding:collapsed?"10px":"10px 12px",border:"none",borderRadius:8,background:on?C.primaryLight:"transparent",color:on?C.primary:C.textSecondary,fontWeight:on?700:500,fontSize:13,cursor:"pointer",textAlign:"left",justifyContent:collapsed?"center":"flex-start"},gabbiStyle)}>
                <span style={{fontSize:18,lineHeight:1,flexShrink:0,width:24,textAlign:"center"}}>{t.icon}</span>
...
const NAV_ICONS = {
  ...
  assistant: <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M19 14c1.49-1.46 3-3.21 3-5.5A5.5 5.5 0 0 0 16.5 3c-1.76 0-3 .5-4.5 2-1.5-1.5-2.74-2-4.5-2A5.5 5.5 0 0 0 2 8.5c0 2.3 1.5 4.05 3 5.5l7 7Z"/><path d="M12 5 9.04 7.96a2.17 2.17 0 0 0 0 3.08c.82.82 2.13.85 3 .07l2.07-1.9a2.82 2.82 0 0 1 3.79 0l2.96 2.66"/><path d="m18 15-2-2"/><path d="m15 18-2-2"/></svg>,
...
const PRIMARY_TABS = ["dashboard","assistant","jobs","covers","profiles"];
    ...
    <nav className="jt-bottom-nav">
      {primaryTabs.map(function(t){
        ...
        return <button key={t.id} className={"jt-bottom-nav-item"+(active?" active":"")} onClick={function(){go(t.id);}}>
          <span className="jt-bottom-nav-icon">{NAV_ICONS[t.id]||null}</span>
